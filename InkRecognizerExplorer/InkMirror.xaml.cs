...
namespace InkRecognizerExplorer
{
    public sealed partial class InkMirror : Page
    {
        ...
        {
            var strokes = inkCanvas.InkPresenter.StrokeContainer.GetStrokes();
            if (strokes.Count > 0 && inkRecognizer != null)
            {
                ...
                else
                {
                    ...
                    {
                        await new MessageDialog("Access denied due to invalid subscription key or wrong API endpoint.", $"Response Code: {inkResponse.Error.code}").ShowAsync();
                    }
                    else
                    {
